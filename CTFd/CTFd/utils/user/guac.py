...
parser.add_argument("--create", action="store_true",
                    help="Create account and add a connection if specified")
parser.add_argument("--delete", action="store_true",
                    help="Delete specified account")
parser.add_argument("--add_connection", action="store_true",
                    help="Add connection for specified user, if coupled with --create, user will be created and assigned the new connection")
parser.add_argument("--delete_connection", action="store_true",
                    help="Delete connection for specified user")
parser.add_argument("--ip", help="Ip Address of the new connection")
parser.add_argument("-u", "--username",
                    help="Username of the account to create or delete")
parser.add_argument("-p", "--password",
                    help="Specify in case of account creation")
...
session = guacamole.session(
    "http://localhost:8080/guacamole", "mysql", "script", "guacadmin")
...
if args.create:
    ...
    createduser = session.create_user(
        args.username, args.password, user_attributes)
    print(createduser)
...
if args.add_connection:
    ...
    try:
        ...
        print(
            f"User {args.username} do not exist, if you wish to create it use --create")
        quit()
